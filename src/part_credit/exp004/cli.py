"""Run EXP004 development or a hash-locked held-out confirmation."""
...
from .experiment import Exp004Config, run_scenario, scenario_suite
from .figures import save_figures
from .statistics import analyze_results

SOURCE_PATHS = (
    "src/part_credit/exp004/__init__.py",
    "src/part_credit/exp004/banks.py",
    "src/part_credit/exp004/environment.py",
    "src/part_credit/exp004/model.py",
    "src/part_credit/exp004/oracles.py",
    "src/part_credit/exp004/experiment.py",
    "src/part_credit/exp004/cli.py",
    "src/part_credit/exp004/figures.py",
    "src/part_credit/exp004/statistics.py",
    "src/part_credit/exp004/frozen_diagnostic.py",
    "tests/test_exp004_mechanisms.py",
    "tests/test_exp004_frozen_diagnostic.py",
    "pyproject.toml",
)
...
def _save(results: dict[str, dict[str, Any]], output: Path) -> None:
    ...
    required = {scenario.name for scenario in scenario_suite(Exp004Config())}
    statistics = (
...
def _validate_confirmation(
    cfg: Exp004Config, names: tuple[str, ...], output: Path
) -> tuple[int, ...]:
    protocol_path = Path("experiments/exp004/FROZEN_PROTOCOL.json")
    if not protocol_path.exists():
        raise SystemExit("confirmation blocked: FROZEN_PROTOCOL.json is absent")
    ...
    if protocol.get("held_out_touched") is not False:
        raise SystemExit("confirmation blocked: held-out state is not pristine")
    if str(output) != protocol["confirmation_output"]:
        raise SystemExit("confirmation output does not match frozen protocol")
    if list(cfg.confirmatory_seeds) != protocol["confirmatory_seeds"]:
        raise SystemExit("confirmatory seeds differ from frozen protocol")
...
def main() -> None:
    ...
    parser.add_argument(
        "--scenarios",
        nargs="*",
        help="Development-only exact scenario names; confirmation always runs all.",
    )
    args = parser.parse_args()
    if args.output.exists():
        raise SystemExit("EXP004 outputs are append-only")
    cfg = Exp004Config()
    available = {scenario.name: scenario for scenario in scenario_suite(cfg)}
    if args.phase == "confirmatory":
        names = tuple(available)
        seeds = _validate_confirmation(cfg, names, args.output)
    else:
    ...
    print(json.dumps({
        name: {
            "success": float(np.mean([row["evaluation_success"] for row in result["seeds"]])),
            "t_alignment": float(np.mean([row["t_alignment"] for row in result["seeds"]])),
        }
        for name, result in results.items()
    }, indent=2))
